<script lang="ts">
    ...
    const checkActive = () =>{
        ...
        if(linkRect.top-linkRect.height/2<viewportHeight/2&&linkRect.bottom+linkRect.height/2>viewportHeight/2){
            active=true;
        }else{
            active=false;
        }
...
<a bind:this={linkRef} class="relative w-fit transition-transform duration-500 ease-fast-slow {active ? "scale-[1.4] translate-x-[20%] md:translate-x-0 md:scale-100":""} {$$props.class || ''}" 
...
>
    ...
    src={activeImage}
    alt="sonder logo"
    class="absolute top-0 left-0 translate-x-[1.05%] scale-[1.02] transition-opacity duration-500 {active ? "opacity-1" : "md:opacity-0"} {$$props.class || ''}"
