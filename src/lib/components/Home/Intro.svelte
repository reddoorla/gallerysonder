<script lang="ts">
  ...
  import image19 from "$lib/assets/images/introImages/intro-19.jpg"
    ...
    for( let i = 0 ; i<1200 ; i++){
        O_paths.push("M"+(70+i*5)+basePath+(70+i*5)+".95Z");
    }
  ...
  $: {
    ...
    if(verticalOpenWindow){
     ...
     if( originalO && animatedOOffestLeft > (innerWidth - animatedOWidth)/2 )
       ...
       if(Math.floor(imageAndPositionArray.length*currentPathIndex/O_paths.length)!=Math.floor(imageAndPositionArray.length*(currentPathIndex+1)/O_paths.length)||currentImageIndex < Math.floor(imageAndPositionArray.length*(currentPathIndex+28)/O_paths.length) || currentImageIndex==imageAndPositionArray.length-1){
        hideO = true;
       }  else{
        hideO = false;
       }
  ...
  <div class="w-screen h-screen bg-black">
    <div 
      ...
      class="h-24 w-[200%] clip-by-logo absolute top-[40vh] use-gpu bg-subtle-primary  {isGrowPhase||!isMounted ? 'opacity-0' : ''}" 
    ...
    <div
        bind:this={animatedO}
        class="h-24 w-[200%] clip-by-logo absolute top-[40vh] use-gpu transition-opacity duration-300 {hideO ? "opacity-0 ":""} {isGrowPhase||!isMounted ? 'opacity-0' : ''}" 
    ...
    <div
      class=" -z-30 absolute w-[200%] bg-black transition-opacity use-gpu h-full {isVerticalOpenPhase ? "max-h-full":"opacity-0 max-h-24 translate-y-[40vh]"}"
    ...
    <div
      class="absolute w-[200%] bg-black transition-opacity use-gpu h-full {isVerticalOpenPhase ? "max-h-full":"opacity-0 max-h-24 translate-y-[40vh]"}"
    ...
    <div 
        ...
        class=" absolute w-[200%] bg-black transition-opacity use-gpu h-full {hideO ? "opacity-0 ":""} {isVerticalOpenPhase ? "max-h-full":"opacity-0 max-h-24 translate-y-[40vh]"}"
    ...
    <div class="h-24 w-[200%] -left-[50%] flex absolute top-[40vh] use-gpu flex-row items-center gap-4 justify-center transition duration-[750ms] {isMounted ? "scale-100 opacity-0 md:opacity-100" : "scale-50"} {isVerticalOpenPhase ? "hidden" : ""}">
        <img src={S} alt="s"  
          bind:this={originalS} 
