<script lang='ts'>
    ...
    function getFontSizeInPixels(element: Element): number {
      ...
      if (fontSizeUnit === 'px') {
        ...
        const parentFontSize = parseFloat(getComputedStyle(element.parentElement as Element).fontSize);
    ...
    $: {
      ...
      if (parent) {
        ...
        nodes.forEach((node) => {
          if (node.offsetWidth > largestChildWidth)
            largestChildWidth = node.offsetWidth;
        });
