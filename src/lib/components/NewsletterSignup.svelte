...
<div class="w-screen h-screen fixed top-0 left-0 z-40  backdrop-blur" transition:fade >
    ...
    <ContentWidth  class="h-full relative flex flex-col items-start justify-center gap-10">
        <button on:click={()=>{$isNewsletterActive=false; $hasNewsletterBeenCleared=true}} class="absolute top-5 left-0"><i class="fa-sharp fa-close fa-2xl text-black" /></button>
        <a href="/" class="absolute top-5 right-0"><RotatingLogo class="h-6" /></a>
        <div>
            <h2>Join Our Community</h2>
            <p>Sign up for our newsletter to receive updates  <br/> on exhibitions, artists, and community events. </p>
        </div>
        <div>
            <input type="text" bind:value={emailValue} class="h-12 pl-2"/>
