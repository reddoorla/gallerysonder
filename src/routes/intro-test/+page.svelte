<script lang="ts">
	...
	const basePath =
		'.44,0c30.24,0,50.49,19.03,50.49,47.52s-20.25,47.52-50.49,47.52H50.49C20.25,95.04,0,76,0,47.52S20.25,0,50.49,0h';
	...
	$: {
		...
		if (originalO && animatedOOffestLeft > (innerWidth - animatedOWidth) / 2) isOCentered = true;

		bgPosition = `calc(${-animatedOOffestLeft}px + ${
			IMAGE_ARRAY_WITH_BG_SHIFTS[currentImageIndex].left
		}vw) calc( ${IMAGE_ARRAY_WITH_BG_SHIFTS[currentImageIndex].top}vh)`;
		currentImageIndex = Math.floor(
			(IMAGE_ARRAY_WITH_BG_SHIFTS.length * currentPathIndex) / O_paths.length
		);

		if (
			Math.floor((IMAGE_ARRAY_WITH_BG_SHIFTS.length * currentPathIndex) / O_paths.length) !=
			Math.floor((IMAGE_ARRAY_WITH_BG_SHIFTS.length * (currentPathIndex + 1)) / O_paths.length)
		) {
			hideO = true;
		} else {
			hideO = false;
	...
	onMount(() => {
		...
		setTimeout(() => {
			...
			ticker = setInterval(() => {
				...
				if (currentImageIndex == O_paths.length - 1) clearInterval(ticker);
			}, TICK_DURATION);
		}, SCALE_UP_PHASE_DURATION_IN_MS);

		if (originalO) animatedOOffestLeft = originalO.offsetLeft;
...
<div class="w-screen h-screen bg-black">
	<div
		class="h-24 w-[120%] clip-by-logo absolute top-[40vh] bg-subtle-primary {isGrowPhase ||
		!isMounted
			? 'opacity-0'
			: ''}"
	...
	<div
		bind:this={animatedO}
		class="h-24 scale-y-110 w-[120%] clip-by-logo absolute top-[40vh] transition-opacity {hideO
			? 'opacity-0 duration-1000'
			: 'duration-50'} {isGrowPhase || !isMounted ? 'opacity-0' : ''}"
	...
	<div
		...
		class=" absolute w-[120%] bg-black transition-opacity h-full {isVerticalOpenPhase
			? 'max-h-full'
			: 'opacity-0 max-h-24 translate-y-[40vh]'}"
		style="background-image: url({IMAGE_ARRAY_WITH_BG_SHIFTS[currentImageIndex]
			.image}); background-size: {IMAGE_ARRAY_WITH_BG_SHIFTS[currentImageIndex]
	...
	<div
		class="h-24 w-[120%] absolute top-[40vh] flex flex-row items-center gap-4 justify-center transition-transform duration-[750ms] {isMounted
			? 'scale-100'
			: 'scale-50'} {isVerticalOpenPhase ? 'hidden' : ''}"
	>
		<img
			src={S}
			alt="s"
		...
		<img
			...
			bind:this={originalO}
			alt="s"
			class="{isGrowPhase || !isMounted ? '' : 'opacity-0'} max-h-screen transition-opacity"
