...
import { W as attr_class, V as attr, X as stringify } from "./index2.js";
const getStores = () => {
...
function LinkArrowButton($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    if (href) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`<a${attr_class(`relative flex flex-row items-center text-center no-underline justify-center transition-all duration-300 active:-translate-y-2 w-fit ${stringify(className)}`)}${attr("target", opensNewTab ? "_blank" : "")}${attr("href", href)}><span class="h-5 uppercase no-underline svelte-rfhjlj">${escape_html(text)}</span> <img${attr("src", linkArrow)} alt="link arrow"${attr_class(`h-5 w-5 ml-[10px] transition-transform duration-300 ${stringify("")}`)}/> <div class="absolute h-[1.5px] bg-black w-5 top-[9px] right-1"></div></a>`);
    } else {
      $$renderer2.push("<!--[!-->");
